...
(0..1).each do |user_num|
	...
	(0..1).each do |article_num|
		article = user.articles.create!(creator: Faker::Name.name ,coverphoto: image_name[2*user_num + article_num] ,view: 0, title: Faker::Lorem.sentence, body: Faker::Lorem.paragraph(10))

		(0..1).each do |comment_count|
